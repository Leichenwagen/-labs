...
class LoginWindow:
    ...
    def login(self): #обработка входа
        ...
        if not login or not password:
            messagebox.showerror("Авторизация", "Пожалуйста, заполните все поля")
            return
    ...
    def add_user(self):#добавление нового пользователя и проверка вводимых данных
        ...
        if not name or not login or not password:
            ...
            if (len(name) < 2) or (len(login) < 5) or (len(password) < 7):
                messagebox.showwarning('Ошибка регистрации', 'Проверьте количество символов, возможно оно меньше допустимого.')
                return
            elif not any(char.isdigit() for char in login):
                messagebox.showwarning('Ошибка регистрации', 'Логин должен содержать хотя бы одну цифру.')
                return
            elif not any(char.isdigit() for char in password) or not any(char.isalpha() for char in password):
                messagebox.showwarning('Ошибка регистрации', 'Пароль должен содержать хотя бы одну цифру и одну букву.')
                return
        ...
        with open("DataFile.json", "r") as file: #чтение файла на поиск схожего
            ...
            for line in lines:
                ...
                if login == stored_login:
                    messagebox.showwarning("Регистрация", "Пользователь с таким именем уже существует")
                    return
    ...
    def open_account_window(self, name): #открытие окна после входа
        ...
        self.logout_button = ttk.Button(self.account_window, text="ВЫЙТИ", command=self.logout, cursor='hand2', width=25).place(relx=0.5, rely=0.55, anchor=tk.CENTER)
